...
class EdifyGenerator(object):
  ...
  def AssertSomeFingerprint(self, *fp):
    ...
    cmd = (' ||\n    '.join([('getprop("ro.product.build.fingerprint") == "%s"') % i
                             for i in fp]) +
           ' ||\n    abort("E%d: Package expects build fingerprint of %s; '
           'this device has " + getprop("ro.product.build.fingerprint") + ".");') % (
  ...
  def AssertFingerprintOrThumbprint(self, fp, tp):
    """Assert that the current recovery build fingerprint is fp, or thumbprint
       is tp."""
    cmd = ('getprop("ro.product.build.fingerprint") == "{fp}" ||\n'
           ...
           '+ getprop("ro.product.build.fingerprint") + " and a thumbprint of " '
